...
## 1. セッション開始の必読（AGENTS.md STEP 1-7 準拠）
...
## 2. Step 1a — 市場データ（マルチペア）+ GMニュース取得
...
- **2026-08-09〜の命名是正と30Y追加**: 旧 `US2Y`(^FVX) は実体が5年債だったため **`US5Y` に改名**（毎週 distilled に「真の2s10s」と補正語を書き続ける必要がなくなった）。**`US30Y`(^TYX) を追加**して `spread_10s30s_bp` / `change_10s30s_bp_1w` / `change_10s30s_bp_30d` / `direction_10s30s_1w` を自動算出。★**10s30s は判定に使わない**（水準・Δ・**符号のみの方向ラベル**を出すだけで、レジームラベル・複合スコアには混ぜない。サンプル4営業日分では閾値を置けないため、8〜12週ためてから検討）。★**Δは必ず窓を明示して引用**（同じスプレッドでも30日と週次で符号が逆になりうる）。★**方向ラベルは週次Δ基準**。★**正本は機械の close-to-close 系列**——手読み値と1本の時系列に混ぜない（理由は**時刻とベンダーの二重の不一致**。手読みはスクショ時刻に依存し、日次1〜3bpの指標では**変化の大半が時刻差**になりうる。※2つの誤差源はまだ分離できておらず、純粋なベンダー差の比較は n=1。「フィード依存だから使えない」ではなく「**まだ分離できていないから混ぜない**」）。★**§8-2「10s30s拡大＝金に追い風」は未検証**——当初の実測根拠が時刻不揃いで無効と判明し、機械系列では**金の週最大上昇日(8/5 +3.67%)に10s30sは縮小**。**仮説として保持し、実測が支持しているという主張はしない**。計数は 2026-08-09 から機械側で開始。★**当日は【同一日・同一チャートのセッション内前後比較】**（発表直前と反応が落ち着いた頃の2点差）。**機械は事後の記録**で、週次/30日窓のΔは当日の動きを拾わない。★スプレッドの符号は全て**市場慣行の【長期−短期】**。★**2026-08-09 改名**: `curve_2s10s`→`curve_spreads` / `change_bp`→`change_bp_30d` / `change_3m10s_bp`→`change_3m10s_bp_30d`（旧名は過去アーカイブに残るが**同一指標の旧名**。snapshot yaml 冒頭に対応行あり）。**真の2Y は Boss 実測（チャート読み）として別レイヤー**に置く＝機械の数値と混ぜない。日本国債（旧 `JP10Y`/`JP2Y`＝米国債の使い回し）は**削除して未取得のまま空けた**。詳細は同 §ティッカー命名の原則と 10s30s の扱い。
...
- ⚠️ **「ライブ検索ツールが無い／独立検証できない」と返る場合の切り分け（2026-08-02 に3週連続失敗の真因を特定）**:
  ...
  - profile 一覧例: `grok`=grok-4.5（**X検索はこれ**）/ `gpt`=gpt-5.6-sol / `Codex`=Codex-opus-4-8 / `ai`,`default`=grok-4.3 / `local`。最新は `hermes profile list` が正。
...
- **Evidence は snapshot 実測値に忠実**。前週比(wkN-1→wkN)を併記。`curve_2s10s`・`intervention_watch` も Evidence/gates に反映。
- **snapshot に JP225 が含まれる**（2026-06-27〜・^N225）。実測の金曜終値が取れる。もし当該週に取得失敗（データ欠落）した場合のみ「金曜終値の実測なし」と書く（創作しない）。
