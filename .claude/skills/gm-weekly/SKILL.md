...
- **2026-08-09〜の命名是正と30Y追加**: 旧 `US2Y`(^FVX) は実体が5年債だったため **`US5Y` に改名**（毎週 distilled に「真の2s10s」と補正語を書き続ける必要がなくなった）。**`US30Y`(^TYX) を追加**して `spread_10s30s_bp` / `change_10s30s_bp_1w` / `change_10s30s_bp_30d` / `direction_10s30s_1w` を自動算出。★**10s30s は判定に使わない**（水準・Δ・**符号のみの方向ラベル**を出すだけで、レジームラベル・複合スコアには混ぜない。サンプル4営業日分では閾値を置けないため、8〜12週ためてから検討）。★**Δは必ず窓を明示して引用**（同じスプレッドでも30日と週次で符号が逆になりうる）。★**方向ラベルは週次Δ基準**。★**フィード差の注意**: 機械(Yahoo)とBossチャート読みは同日で最大約1.9bpずれ、**これは週次Δと同オーダー＝符号がフィード依存**になる（7/31→8/7 は機械+2.1bp/widening vs Boss実測−0.5bp/narrowing）。**機械フィード内で完結して読み、Boss実測と1本の時系列に混ぜない**（WTIと同じ既知の乖離扱い）。★**当日はチャート読み・機械は事後の記録**（週次/30日窓のΔは当日の動きを拾わない）。★スプレッドの符号は全て**市場慣行の【長期−短期】**。**真の2Y は Boss 実測（チャート読み）として別レイヤー**に置く＝機械の数値と混ぜない。日本国債（旧 `JP10Y`/`JP2Y`＝米国債の使い回し）は**削除して未取得のまま空けた**。詳細は同 §ティッカー命名の原則と 10s30s の扱い。
