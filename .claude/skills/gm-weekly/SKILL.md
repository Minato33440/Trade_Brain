...
- **2026-06-27〜の構造改善（3項目）**: ①JP225（^N225）を実測パネルに追加＝**JP225も機械実測で取れる**ようになった、②`curve_2s10s`＝yieldsラベルの丸めを補正するベアフラット検知、③`intervention_watch`＝IMF残弾/会談/単独協調フラグ（`configs/settings.py` の `INTERVENTION_WATCH` を手動更新）。詳細は `docs/WEEKLY_UPDATE_WORKFLOW.md` Step 1a §スナップショットの構造改善。
