...
| AGENTS.md の主眼 | 凍結ファイル保護・API 整合 | スキーマ遵守・Wiki 同期・週次運用 |
...
| Advisor | Codex.ai（Opus 4.7 等） | 蒸留・構造化・Wiki 更新提案 |
| 実装 | Codex（Sonnet 4.6） | Ingest 実行・週次更新・ファイル操作 |
...
- `~/.Codex/AGENTS.md` の指示経由で Codex が手動で `rtk` を付ける方式が唯一の動作モード
- 詳細: `docs/WEEKLY_UPDATE_WORKFLOW.md` §0（RTK 使用ルール）

### RTK の適用範囲（重要 / 2026-04-25 明記）

- **RTK は Codex（ローカル端末）専用ルール**
- **Codex.ai セッション（Advisor 等）が GitHub MCP を使う場合は RTK 不要**
...
│
├── coordination/               # 両利きレーン: executor→Broker の append-only handoff（Provider3分岐・出力専用）
│   ├── claude/                 #   例: claude/<YYYY-MM-DD-hhmm>_<topic>.md
│   ├── codex/
│   └── grok/
├── scratch/                    # 両利きレーン: engine別・使い捨て作業域
│   ├── claude/                 #   instructions/ に指示書（入力）を置く
│   ├── codex/                  #   例: codex/instructions/<topic>.md
│   └── grok/
│
...
```
RTK ルール:             WEEKLY_UPDATE_WORKFLOW.md > AGENTS.md
命名規則（distilled）:  WEEKLY_UPDATE_WORKFLOW.md §4 STEP 7 = AGENTS.md 命名規則（一致）
週次手順:               WEEKLY_UPDATE_WORKFLOW.md > Trade-Main.md > AGENTS.md
...
### coordination/ ・ scratch/ — 両利きレーン（2026-07-04 追加 / 2026-07-05 Provider3分岐）

```
位置づけ: canon（distilled/）の手前の実務レーン。Codex / Claude 双方が扱える（両利き）。
          canon 生産（distill）・index 昇格とは別層で、可逆・append-only。
          両レーンとも Provider 3分岐（claude/ codex/ grok/）で engine をフォルダが担う。

coordination/ — 出力専用（下流ハンドオフ）:
  - executor→Broker の handoff 記録。append-only。
  - coordination/<engine>/<YYYY-MM-DD-hhmm>_<topic>.md（1エントリ=1ファイル）
  - engine はフォルダで表すためファイル名に <engine>_ は付けない（二重回避）。
  - 単一ファイルへの共有追記はしない（両engine同時書き込みの last-write-wins 回避）。

scratch/ — 作業域（入力・中間生成物）:
  - engine別の使い捨て作業域。scratch/<engine>/。
  - 生素材抽出・index feed 再編・逆引き通し等の中間生成物。
  - 指示書（Boss/Broker→executor の入力）は scratch/<engine>/instructions/ に置く。
    ※ 指示書は handoff ではなく入力なので coordination には置かない（流れを一方向に保つ）。
...
書き込み権限:
  - Codex が自動書き込み可能: Signals/ / Events/ / Instruments/
...
10. GitHub MCP 接続リポの canon 層（distilled/ / docs/ / Strategy_Wiki/ / ルート governance）は
    ...
    ── 例外: scratch/ と coordination/ は両利きレーン（Codex / Claude 双方可）とし
       ...
       （過去2件の連鎖失敗事故の再発経路を塞ぐため）。両レーンとも Provider 3分岐
       （claude/ codex/ grok/）で engine をフォルダが担う。coordination/ は
       coordination/<engine>/<YYYY-MM-DD-hhmm>_<topic>.md（1エントリ=1ファイル）とし
       単一ファイルへの共有追記をしない（last-write-wins 回避）。
       指示書（入力）は scratch/<engine>/instructions/ に置き coordination には置かない。
```
...
**⚠️ 週末 Git 更新の依頼を受けたら、Codex 自身が最初に実行する（依頼不要 / 2026-05-16〜）:**

```bash
python main.py --trade --news < /dev/null
```

- configs/rex_chat.py 修正済（2026-05-16）: XAI_API_KEY なしでも `--trade --news` 動作・stdout UTF-8固定・データ後 exit 0。
- news はライブ取得のため**週次更新開始時に1回実行し最新出力を確定採用**（ボス 2026-05-16）。
- 失敗（exit≠0・データ欠落）時は**不変ルール7に従い想像補完せず停止・報告**。
- **ボス提供の1次資料は #1 市況 / #4 口座 / #5 トレード結果 の3点のみ**（#2 --trade / #3 --news は自動取得）。

理由: 実測値なしで作成すると、推定値で作成後に実データで再更新する二度手間が発生する。
...
coordination / scratch（2026-07-04 追加 / 2026-07-05 Provider3分岐）:
  coordination/<engine>/<YYYY-MM-DD-hhmm>_<topic>.md   例: coordination/codex/2026-07-05-1830_reverse_lookup_dryrun.md
    （1エントリ=1ファイル・出力専用・engine はフォルダが担いファイル名に <engine>_ は付けない）
  scratch/<engine>/{任意}.md                            例: scratch/claude/2026-7-3_wk01_prediction_seed.md
  scratch/<engine>/instructions/<topic>.md              例: scratch/codex/instructions/reverse_lookup_dryrun.md（指示書=入力）
...
charts（週次フォルダ内）:
  ...
  charts/GM Strategy-YYYY-M-D.txt           （Codex 作成）
```

---

## Git コミット手順（Codex のローカル作業向け・RTK 必須）
...
*.png                  → 追跡対象（1次プロット資料として版管理。.gitignore除外しない / 2026-05-16〜変更）
```
...
**両利きレーン例外（scratch/ ・ coordination/ / 2026-07-04 追加・2026-07-05 Provider3分岐）**: canon ではないため
上記に依らず filesystem write 可。ただし新規作成・append のみ（edit_file 禁止）。両レーンとも
Provider 3分岐（claude/ codex/ grok/）。coordination/ は coordination/<engine>/<YYYY-MM-DD-hhmm>_<topic>.md
（1エントリ=1ファイル・出力専用）。指示書（入力）は scratch/<engine>/instructions/ に置く。
Codex / Claude 双方に等しく適用。
...
例外 — 両利きレーン（scratch/ ・ coordination/ / 2026-07-04 追加・2026-07-05 Provider3分岐）:
- `write_file`（新規作成）・append は可。`edit_file` は canon 同様に不可。
- 両レーンとも Provider 3分岐（claude/ codex/ grok/）。
- coordination/ は coordination/<engine>/<YYYY-MM-DD-hhmm>_<topic>.md（出力専用）。
- 指示書（入力）は scratch/<engine>/instructions/ に置く（coordination には置かない）。
...
- **Codex（ローカル）**: すべての `git` コマンドに `rtk` 必須
- **Codex.ai Advisor（GitHub MCP）**: ローカル `git` コマンドを通さないため **RTK 不要**
...
詳細は `Minato33440/Trade_System/AGENTS.md` の §役割別の編集経路 を参照。要約:
...
Rex 記憶層:     C:\Python\REX_AI\REX_Brain_Vault\REX\Rex-Vault.md
                データと検証は本リポへ、判断の驚き・跨ぐパターンは Vault へ（義務ではない）
...
| 2026-07-05 | coordination/ を Provider3分岐（claude/codex/grok/）化・命名から <engine>_ 除去。指示書置き場を scratch/<engine>/instructions/ に規定 |

---

## 発行

作成: 2026-04-18（Advisor / Codex Opus 4.7 提言による）
更新履歴:
  - 2026-04-18 朝: 初版
  ...
  - 2026-07-05: coordination/ を Provider3分岐化・命名から <engine>_ 除去（engineはフォルダが担う）。指示書は scratch/<engine>/instructions/ に規定（handoffでなく入力のため coordination に置かない）
  - 2026-07-09: 外部リソース参照先に Rex 記憶層ポインタを追加（入口: REX\Rex-Vault.md）
