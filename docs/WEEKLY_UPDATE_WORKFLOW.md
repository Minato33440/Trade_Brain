...
2. **`curve_spreads` セクション**（金利カーブの**4点立体** 3M/5Y/10Y/30Y / 2026-06-27 拡張・**2026-08-09 に30Y追加＋改名**）: `spread_bp`（5s10s=US10Y−US5Y[^FVX]）/ `change_bp_30d` / `shape` ＋ **`spread_3m10s_bp`（3m10s=US10Y−US3M[^IRX]＝Fed重視の景気後退カーブ・逆イールド主ゲージ）** / `change_3m10s_bp_30d` / `shape_3m10s` / `spread_3m5s_bp` / `belly_premium_bp`（5Yの直線補間からの突出度）/ `structure`（`belly_elevated`=front政策<belly5Y突出<long の瘤 等）/ `recession_3m10s`（`positive`/`near_inversion`<25bp/`inverted`）/ `points_pct`（m3/y5/y10/**y30**）＋ **`spread_10s30s_bp` / `change_10s30s_bp_1w` / `change_10s30s_bp_30d` / `direction_10s30s_1w`**。<br>★**2026-08-09 改名**: `curve_2s10s`→**`curve_spreads`**（中身が 5s10s/3m10s/belly/10s30s で 2s10s を1つも含まなかったため）/ `change_bp`→**`change_bp_30d`** / `change_3m10s_bp`→**`change_3m10s_bp_30d`**。**旧名は 2026-08-09 以前の distilled・週次アーカイブに残っている**が、同名の別指標ではなく**同一指標の旧名**（snapshot yaml 冒頭にも対応行を出力）。**`yields` ラベルは5Y/10Y平均符号の丸めで、ベアフラット（短期↑/長期↓）を見落とす**ため、その補正指標。**読み方**: `shape`は各区間のフラット化の質、**逆イールド接近の本格警戒は `recession_3m10s` で判定**（5s10sは順イールド環境で最フラット区間＝逆イールド接近を過大評価しうるため）。`structure=belly_elevated`＝政策ターミナル織り込みの瘤（front=政策3M / belly=5Y突出 / long=growth 10Y）。
...
※ `curve_spreads`（旧 `curve_2s10s`）は4点（3M/5Y/10Y/30Y）を併記。`US3M=^IRX`(13週) / `US5Y=^FVX` / `US10Y=^TNX` / `US30Y=^TYX`。5s10sは順イールド環境で最フラット区間のため逆イールド接近を過大評価しうる→**主ゲージは `recession_3m10s`**（yamlの`note`参照）。

#### ティッカー命名の原則と 10s30s の扱い（2026-08-09 / ボス指摘）

**「X」と「真のX」を並記し続けないと成立する指標は、名前の側が間違っている。** 代用品には本物の名前を付けず、取れないものは「未取得」として空けておく。**欠損は気づかれるが、間違った値は気づかれない**。

| 変更 | 内容 |
|---|---|
| **改名** | `TRADE_PAIRS` / `CORE_TICKERS` の **`US2Y: ^FVX` → `US5Y: ^FVX`**。`^FVX` の実体は5年債で、毎週 distilled に「**真の2s10s**」という補正語を書き続ける羽目になっていた。改名により補正語が不要になる |
| **追加** | **`US30Y: ^TYX`**（4点目）。Yahoo系で取れる米国債の指数は 3M/5Y/10Y/30Y で、**2Y の指数は存在しない**。→ 機械は4点を正直な名前で持ち、**真の2Y は Boss の実測（チャート読み）として別レイヤー**に置く |
| **削除** | `CORE_TICKERS` の **`JP10Y: ^TNX`** と `FULL_TICKERS` の **`JP2Y: ^FVX`**。日本のラベルで米国の値を返しており、対話モードで日米金利差を出すと **0.00bp** になって「壊れて見えない」形で誤った。日本国債は取得手段がないため**未取得のまま空ける** |

#### ★ FRED は別フィード — インフレ補償（BE）の機械取得（2026-08-20 / ボス指示）

**代理をやめて本体を取りに行く。** 年輪候補 **8-2**「金の上昇には *ドル安 かつ BEが崩れないこと* の両方が要る」の**第二条件が、一度も実測されていなかった**。代理を2回立てて2回とも失敗している。
...
→ **原油とBEは連動していない。** 原油をBEの代理に使うのは、10s30sを使ったのと同じ誤り。

##### これは `TRADE_PAIRS` への1行追加ではない — データソースが違う

| | ソース | 経路 |
|---|---|---|
| `TRADE_PAIRS` / `CORE_TICKERS` | Yahoo Finance ティッカー（`^IRX` `^FVX` `^TNX` `^TYX`） | `src/data_fetch.fetch_market_data` |
| **`FRED_SERIES`** | **FRED のシリーズID**（`T5YIE` `T5YIFR`）。**Yahooには存在しない** | **`src/market.fetch_fred_snapshots`（別関数）** |

⚠️ `TRADE_PAIRS` にそのまま入れると、Yahooフェッチャが `T5YIE` をティッカーとして解決しようとして失敗する。**同じ辞書・同じ経路に混ぜない。**

取得は認証不要：`https://fred.stlouisfed.org/graph/fredgraph.csv?id=<SERIES_ID>`

**実装上の注意3点（2026-08-20 実測で確認）:**

1. **ヘッダは `observation_date,<SERIES_ID>`**（`DATE` ではない）→ **列名に依存せず位置で読む**
2. **欠損は空フィールドで返る**（祝日の行は存在し、値だけが空）。旧APIの `.` も念のため許容 → `float()` に渡す前に弾く
3. **非営業日を含むので、窓は必ず【日付基準】**（`iloc[-7]` のような位置指定は不可。`prev_1w` と同方針）

##### ★ 公表ラグ — `as_of` を必ず確認する

**FREDのBEは、Yahooの価格データより1日古い可能性が常にある。** 黙って同じスナップショットに並べると、**「測定窓の不一致」を機械に焼き付ける**ことになる（8月に7回繰り返した失敗）。
...
inflation_compensation:
  ...
  us5y_be_pct: 2.28                # 水準は%表記（bpではない）
  change_5ybe_bp_1w: 4.0           # 週次Δ（bp）
  change_5ybe_bp_30d: 0.0          # 30日Δ（bp）
  direction_5ybe_1w: widening      # 週次Δの符号のみ。閾値は未設定＝判定はしない
  us5y5y_be_as_of: 2026-08-19
  us5y5y_be_pct: 2.32              # 5年5年先フォワード＝長期インフレ期待アンカー
  change_5y5ybe_bp_1w: 4.0
  change_5y5ybe_bp_30d: 10.0
  direction_5y5ybe_1w: widening
```

**`as_of` が他ブロックの日付と一致しない場合、それは異常ではなく正常。** 人間とLLMが読んだときに気づけることが要件。

##### 2本ある理由（`T5YIE` と `T5YIFR`）

議事録が言っているのは「**近期の**インフレ補償」が動かなかったということ。近期（5Y BE）はエネルギー価格の影響を受けやすく、**5y5yフォワードはそれを剥いだアンカー**。**2つの差が診断になる**：

- **5Y BEだけ動いて5y5yが動かない** → 市場は原油の影響を**一時的**と見ている
- **両方動く** → **インフレ期待そのもの**が動いた

**8-2が問いたいのは後者**なので、1本だけでは判別できない。

##### ★ やってはいけないこと

1. **レジームラベル・複合スコアに入れない。** `_yields_regime()` にも他のどの合成指標にも混ぜない（US30Y を混ぜなかったのと同じ理由）。**サンプルがゼロの指標に判定させると、判定が実測より先に固まる＝8-2で2回やった失敗そのもの。**
2. **閾値を置かない。** `direction_5ybe_1w` は**符号のみ**。「BEが崩れた」の水準を今決めない。**8〜12週ためてから検討する。**
3. **原油との相関を自動計算しない。** 議事録の観測は**1期間分の1事例**で、まだ自分で確かめていない。相関を機械が出すと**検証済みの事実として扱われる**。まず生の系列をためる。
4. **例外を握り潰さない。** FRED取得は外部HTTPなのでYahooより失敗しやすい。**「データ欠損」と「取得失敗」を区別できる形で出す**：

```yaml
inflation_compensation:
  # ERROR: URLError: <urlopen error ...>（データ欠損ではなく取得失敗）
  status: fetch_failed
```

同じ理由で `rex_chat.py` の `--trade` 側も是正した。**以前は例外を吸って stderr にだけ出し、`--trade` は完走するのに snapshot が丸ごと欠落する経路があった**（2026-08-09 の `_curve_2s10s is not defined`）。現在は **LLM に渡すテキストにも `[regime] ERROR: ...` を明示し、失敗を記録した snapshot も書き出す**。

> `settings.py` 冒頭の原則「欠損は気づかれるが間違った値は気づかれない」は、**欠損が欠損として見えるときにしか成り立たない。**

#### スプレッドの符号は【長期 − 短期】で統一（2026-08-09）

市場慣行に合わせ、`spread_bp`（5s10s）/ `spread_3m10s_bp` / `spread_10s30s_bp` は**すべて「長期 − 短期」**で計算する。**2s10s が +45bp とは「10Y が 2Y より 45bp 上」**の意味。

⚠️ 指標名（"5s10s"）を読み順のまま引き算に写すと逆になる。**以前はコード側が正しく、コメント側が3本とも逆順**（`US5Y − US10Y` 等）に書かれていた（2026-08-09 に是正）。**「コメントに合わせてコードを直す」と符号が反転**し、`direction_10s30s_1w` の widening/narrowing が入れ替わる。**絶対値は同じなので目視では気づけない**——ラベルが実体を上書きする型の、もう一段下の層の再発だった。

#### `10s30s` は判定に使わない（2026-08-09 時点）

出すのは以下の**4フィールドだけ**で、レジームラベルにも複合スコアにも混ぜない（`_yields_regime()` にも `US30Y` は入れていない）：

| フィールド | 内容 |
|---|---|
| `spread_10s30s_bp` | 水準（US30Y − US10Y） |
| `change_10s30s_bp_1w` | **週次Δ**（7暦日前【以前】の最後の終値が基準。祝日で営業日数が変わるため位置指定ではなく日付基準） |
| `change_10s30s_bp_30d` | **30日Δ**（構造的ドリフト） |
| `direction_10s30s_1w` | **週次Δの符号のみ** = `widening` / `narrowing` / `flat` |

**窓をフィールド名に入れている理由**: 同じスプレッドでも**窓が違えば符号が逆になりうる**（30日では拡がり、直近1週では縮む等）。窓を書かないと、符号が逆の2つの記述が同じ distilled に並び、読み手が毎回「どちらの窓か」を意識する羽目になる——「真の2s10s」を毎週書いていたのと同じ構図の再発。**引用するときは必ず窓を明示すること。**

**方向ラベルを週次にした理由**: §8-2 の読み筋を導いた観測が日次〜週次の動きだったため。**30日Δで方向を出すと、過去1ヶ月のトレンドが直近の変化を上書きする。**

**閾値を置かない理由**: **サンプルが4営業日分しかない**。線を引くには足りず、水準に意味を割り当てるには全然足りない。**8〜12週ためてから閾値の検討を行う**（それまでは人間側で読む）。

#### ★ 10s30s の正本は【機械の close-to-close 系列】（2026-08-09 確定）

手読みの値と機械の値を**1本の時系列に混ぜない**。理由は**時刻とベンダーの二重の不一致**である。

⚠️ **2026-08-09 に判明した実例（当初 §8-2 の根拠にしていた4点が、時系列として無効だった）**：
...
**機械の close-to-close 系列で見ると結論が変わった**：
...
→ **金が週で最も上げた日に 10s30s は縮小していた。**「8/5に拡大・金+3%」は成立しない。

**誤差源の分離はまだできていない**（下表）。純粋なベンダー差の比較は**同一時刻同士の n=1 のみ**なので、「フィード依存だから使えない」と結論づけるのは早い。
...
一方、**機械の系列内**の日次変動は +3.3 / −0.6 / −1.4 / +0.8 で、週次Δ +2.1 はその範囲に収まる。**同一系列内で見れば信号がノイズに埋もれているとは言えない。**

> ❌「10s30s の週次Δの符号はフィード依存」← **診断が早い（n=1）**
> ✅「**異なる時刻・異なるベンダーの値を混ぜると符号が反転する。混ぜるな**」

運用上の結論は同じ（混ぜない）だが、**分離できれば併用可能になる余地は残す**。

**計数は機械側で行う**: 機械は毎日同じ時刻・同じベンダーで自動的に積み上がるが、手読みはスクショを撮った時刻に依存する。**8〜12週ためる計画は、一貫した系列でなければ意味がない**ので、**2026-08-09 から機械側で数え始める**。手読み4点は**導出の経緯**（なぜ10s30sを見ようと思ったか）としては残すが、**時系列としては破棄**。

**読み筋（人間側・★未検証）**: **拡大＝タームプレミアム／ソブリン信用の象限で金に追い風** ／ **縮小＝純粋なディスインフレで金には何も来ない**。機構の議論（30Yが粘る＝準備資産分散 / 30Yも下がる＝純ディスインフレ）は独立に立つが、**2026-08-09 時点で機械系列による実測の裏づけは無い**（7営業日で符号一致5/7だが、金の最大上昇日 8/5 が外れる）。**仮説として保持し、実測が支持しているという主張はしない。**

#### ★ 当日は【セッション内の前後比較】、機械は事後の記録（手順の分離）

**この指標で指標発表当日の反応は `--trade` では判定できない。** Δは週次／30日窓なので**当日の動きをほぼ拾わない**。

かといって「当日にチャートを1回読んで水準を出す」のも**足りない**——21:31に読んだ値は、機械のNY引け系列とも、他の日のどの値とも比較できない。**同一日・同一チャート内の前後比較**にする。

| タイミング | 手段 |
|---|---|
| **当日・発表直前（例 21:25頃）** | US10Y / US30Y を読み、**30Y − 10Y を記録** |
| **当日・反応が落ち着いた頃（例 23:00頃）** | **同じチャートで再度読む** |
| **判定** | **その2点の差**を見る（開いたか / 閉じたか） |
| **週末（`--trade`）** | 機械が週次・30日の構造的ドリフトを記録する（事後の記録） |

同じ日・同じ時間帯・同じベンダーなので、**時刻差もベンダー差も入らない**。求めているのは「**CPIの反応で10s30sが開いたか閉じたか**」という**当日の変化**であって水準ではないので、これで十分。

⚠️ **当日の夜に `--trade` を回して `widening` を見て、それが1ヶ月前（または前週）との比較だと気づかないまま判断する経路を作らないこと。**
...
### 2.1.1 ★ snapshot を読む前に必ず見る3つの欄（2026-09-06 追加）

`--trade` の出力と snapshot yaml には、**値そのものより先に見るべき欄**がある。
どれも**過去に実際に事故った層**を塞ぐために足したもので、**黙って通り過ぎると値が正しく見える。**

| 欄 | 何を守るか | 由来 |
|---|---|---|
| `snapshot_date_integrity` | ★**ペアごとに `latest` の日付が割れていないか**（`aligned` / `SPLIT` / `missing`）。**SPLIT の週は、ペアをまたぐ差分が【異なる時点の引き算】になる** | 2026-08-28（金利4本だけが講演前の 8/27 だった） |
| `value_revisions` | ★**確定済みの日付の値が、前回取得時から変わっていないか**。**日付は正しいのに値が変わる**故障は、`as_of` を見ても分からない | 2026-09-06（`GC=F` が同じ 9/4 の終値として +46.80 の差を返した） |
| `change_*_bp_1w_window` | ★**週次Δが実際に引き算した2日**。7暦日ルックバックは祝日・欠測で落ちる日が動き、**その1日の出入りだけで符号が反転しうる** | 2026-09-06（`curve_fred` の窓が 8/28 を跨がず、機械 -4.0bp / Boss 窓 +4.0bp） |

★ **`off_calendar_*` に出ている銘柄（BTC/USD 等 24/7）は、日付が違って正常。** 異常ではないので `SPLIT` には数えない——
**常時点灯する条件は、警告ではなくメタデータとして持つ**（年輪 §8(g-2)）。

★ **`value_revisions` が出たら、機械が「どちらが正しいか」を決めることはしない。**
**変わったという事実を出すだけ**で、採用する値は人間が決める（台帳は初回値を上書きしていない）。
...
### 2.2.1 ★ 口座の前週比は「比較可能ベース」を併記する（2026-09-06 恒久ルール）

★★ **銘柄の入れ替えがあった週は、評価損益の前週比をそのまま引かない。**
**売却した銘柄の含み損益が前週の数字に入っているため、前週比に構成変化が混ざる。**

実例（2026-09-01 に 513A を売却した週）:

| | 値 |
|---|---:|
| wk04 評価損益 | +450,101（★**513A の含み損 -16,500 を含む**） |
| **513A を除いた比較可能ベース** | **+466,601** |
| wk01 評価損益 | +321,241 |
| ヘッドラインの前週比 | -128,860 |
| ★**比較可能ベースでの前週比** | ★**-145,360** |

★ **含み損が実現に移った分だけ、ヘッドラインは見かけ上「改善方向」に働く。** この週の差は約 16,500 円。

→ **売却・買い増しがあった週は、ヘッドラインと比較可能ベースの両方を出す。**
⚠️ **買い増しがあると厳密な同一構成比較にならないので、概算であることを明示する。**
（年輪 §8(g-3)）
...
## 7. 追記・持ち越しの引継ぎ確認（2026-09-12追加）

根拠: 2026-9-11_wk02で、Bossが後から提供したRex追記を本体へ反映した際の確認事項。[当週の反映記録](../logs/weekly/2026/2026-9-11_wk02/rex_addendum_integration.md)。既存手順を保ち、次回の入力確認と出力照合に以下を追加する。

- 入力開始時にレポート末尾のRex/Astra追記の有無・日付・範囲を確認し、前週trade_results§9と最新carry_forwardを読み合わせる。追記が後着したら初回入力ハッシュを維持し、新しい版・理由・受領時刻を記録する。
- carry_forwardをID単位で継承し、完了/運用上解決/未決/撤回を分ける。旧状態を黙って再利用しない。当週の成果と残る課題を次回指示書へ出力する。
- キャンペーン別の約定行と件数を照合する。新規建て/部分決済/全決済/累計行を混ぜず、実現損益＋含み損益変化＝評価込み増分を未丸め値で検算する。契約倍率未確認ならpt・Lotから金額換算しない。
- trigger/action/invalidationを対で確認する。双方向の仮説には双方向の反証を記述し、規則変更の根拠・適用日・旧規則・カウント対象を保存する。訂正前の週を無断で再集計しない。
- instrument/vendor/as_of/窓/cutoffを添える。不明なcutoffは不明と記載。ETFの国内引け、海外原資産、CFD/FX、継続先物、FRED公表日を同期データとして扱わない。
- value_revisionsは初回/改訂を保持し、過去記録と照合して一方向/往復/不足を区別する。価格パターンだけで原因を確定せず、実ティッカーと限月切替が不明なら未決とする。既存検出器は初回値比較であり、直前観測比較・戻り検出は未実装。
- 外部レビューは読了ファイルと未読ファイルを記録。提供追記も数値・制度・因果を検証し、訂正を新しい日付で残す。

次回に使う開始資料: [全持ち越しID](../logs/weekly/2026/2026-9-11_wk02/carry_forward.md) / [次回指示書](../logs/weekly/2026/2026-9-11_wk02/next_update_handoff.md)。以降は各週が更新した同名資料を引き継ぐ。
