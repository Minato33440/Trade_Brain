...
30Y を入れると `curve_2s10s` に 10s30s を併記できるようになるが、**新しいレジームラベルや複合スコアには混ぜていない**。出すのは3つだけ：

- `spread_10s30s_bp`（水準）／ `change_10s30s_bp`（Δ）／ `direction_10s30s`（**符号のみ** widening / narrowing / flat）
