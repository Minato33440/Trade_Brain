# GM Weekly Review — 2026-1-3_wk52
period: 2025-12-29 → 2026-01-03
updated: 2026-01-03 (JST)
scope: what mattered / regime / week-ahead playbook

## Summary
年末ラリー期だが、実態は「高値圏の消化（小幅レンジ）」。
株が崩れていない一方でGoldが強く、ヘッジ需要が残る“ヘッジ付きリスクオン”の形。

## What mattered（cross-asset）
- US100: 上昇トレンドは維持。ただし高値圏で失速＝次は“ブレイクの質”チェック。
- JP225: 強い（高値圏維持）。USDJPYの向き次第でβが変わる。
- USDJPY: 高値圏でコイル（方向待ち）。154.7割れは日本βの警戒スイッチ。
- XAUUSD: 上昇トレンド保ち、押しでも崩れない＝ヘッジ需要が消えていない。
- BTC: 下落後のベース作り。今は“先導役”というより“地合いセンサー”。

## Regime call（rule-based）
- Risk-on（cautious）/ thin liquidity.
- Key risk: 「低ボラ × 薄商い」→ ある日突然、逆回転（ギャップ）しやすい。

## Next-week playbook（portfolio）
### Core（維持）
- 1655 / 201A / 425A：コアは維持（増減は“US100の確認”待ち）

### AI beta（2243 / 2638の扱い）
- 増やす条件（追わない）
  - US100 > 25,670 を日足で上抜け＋21MAサポート化
  - その後の押しで再構築（ブレイク当日は追わない）
- 守る条件
  - US100 < 23,692（日足終値）→ 追加停止＋現金/Gold寄せ
  - USDJPY < 154.7（日足/週足）→ 日本β（2243/2638/JP個別）から先に圧縮検討

### Hedge / risk control（優先順位）
- まず「新規追加を止める」→ 次に「βの高い順に圧縮」→ 最後にコアを触る
