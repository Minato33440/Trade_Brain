...
## 役割
お前は retrieval と観測と照合だけを行う。設計判断はしない——粒度の是非は所見として Broker(claude) に返すだけだ。Vault index/（bridges\trade_brain\index\）には書かない・触らない（read のみ）。照合できない部分は未照合と明記しろ。お前の担当はここまでだ。
...
## 出力
- outcome_record: scratch/codex/2026-6-19_wk03_reverse_lookup_std.md（v1 の outcome_record スキーマに準拠、先行specimen 参照）
- 記録: coordination/codex/<YYYY-MM-DD-hhmm>_outcome_6-19_std.md（1エントリ=1ファイル）
